...
def detect_damage(aoi, inference_start, war_start, pre_interval=12, post_interval=2, footprints=None, viz=False, export=False, export_dir='PWTT_Export', export_name=None, export_scale=10, grid_scale=500, export_grid=False, clip=True, method='stouffer', threshold=3.3, ttest_type='welch', smoothing='default', mask_before_smooth=True, lee_mode='per_image', filter_builtup=True):
    ...
    if filter_builtup == "esa_cover":
        land_cover = ee.Image("ESA/WorldCover/v200/2021");
        builtupMask = land_cover.eq(50) # 50 = built-up class
        image = image.updateMask(builtupMask)
        
    elif filter_builtup == "ghs_cover":
        landcover = ee.Image('JRC/GHSL/P2023A/GHS_BUILT_S/2020').select('built_surface')
        builtupMask  = landcover.gt(0)
        image = image.updateMask(builtupMask)

    elif filter_builtup == "ghs_pop":
        pop = ee.Image('JRC/GHSL/P2023A/GHS_POP/2020')
        builtupMask  = pop.gt(1)  ## this should be taken with caution
        image = image.updateMask(builtupMask)

    elif filter_builtup == "google_cover":
        ## Note that the latest builtup presence layer from google is 2023
        landcover = ee.ImageCollection('GOOGLE/Research/open-buildings-temporal/v1').filterDate("2023-01-01", "2023-12-01").filterBounds(aoi).select('building_presence').mosaic()
        builtupMask  = landcover.gte(0.1)
        image = image.updateMask(builtupMask)
    else:
        print("Computed image will be exported without masking by extra layer")
    ...
    # bands = image.toBands()
    # arr = geemap.ee_to_numpy(image, region=aoi) # , bands=['damage','B3','B2']
    # if not export:
    #     return arr
    # else:
    ...
    if footprints is not None:
        print(f"Creating regional summary using footprints from {footprints} ....")
        if footprints == "google":
            fc = ee.FeatureCollection('GOOGLE/Research/open-buildings/v3/polygons').filter('confidence >= 0.5').filterBounds(aoi)
            sub = image.select("damage") # .selfMask()
            fp = sub.reduceRegions(
                collection=fc,
                reducer=ee.Reducer.anyNonZero(), ## if there is a single pixxel flagged as damage within a polygon, its considered as damaged. So the outcome will be 0 no-damage and 1 damaged 
                scale=10,
                tileScale=8,
            )
        else:
        ...
        print(f"Exporting regional summary table creating using footprints from {footprints} ....")
    ...
    if export:
        print("Exporting the image to drive----")
